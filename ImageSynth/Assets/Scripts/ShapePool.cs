...
public class ShapePool : ScriptableObject
{
    ...
    // prefab array each type pf prefab will have different pool of cached objects

    public Dictionary<ShapeLabel, List<Shape>> pools; 
    public List<Shape> active;
    ...
    public Shape Get(ShapeLabel label){
        ...
        if (lastIndex <= 0){
            var obj = Instantiate(prefabs[(int)label]);  // type casting
    ...
    // ReclaimAll - looks at all the active objects and makes them inactive to remove them from the screeen.
